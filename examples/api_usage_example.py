...
class WatermarkerClient:
    ...
    def upload_and_watermark(
        self, file_path: str, text: str = "WATERMARK", position: str = "top-left"
    ) -> Optional[dict]:
        ...
        try:
            with open(file_path, "rb") as f:
                ...
                response = requests.post(
                    url, files=files, data=data, headers=self.headers
                )
...
def main():
    ...
    result = client.upload_and_watermark(
        file_path=FILE_TO_PROCESS, text="SAMPLE_WATERMARK", position="bottom-right"
    )
