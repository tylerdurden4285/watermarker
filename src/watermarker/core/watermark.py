...
def ensure_directory(path: str, mode: int = 0o775) -> None:
    """Create a directory with desired permissions and ownership."""
    os.makedirs(path, mode=mode, exist_ok=True)

    uid_env = os.getenv("USER_ID")
    gid_env = os.getenv("GROUP_ID")
    if uid_env or gid_env:
        try:
            uid = int(uid_env) if uid_env else -1
            gid = int(gid_env) if gid_env else -1
            os.chown(path, uid, gid)
        except (PermissionError, AttributeError, ValueError):
            # Ignore if we lack permissions or running on a platform without chown
            pass
...
def load_config() -> Dict:
    ...
    config = {
        ...
        "font_file": get_env_var(
            "FONT_FILE", "/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf"
        ),
        "upload_folder": get_env_var("UPLOAD_FOLDER", "./uploads"),
        "max_upload_size": int(get_env_var("MAX_UPLOAD_SIZE_MB", "1024"))
        * 1024
        * 1024,  # Convert MB to bytes
    }

    # Create upload folder with appropriate permissions
    ...
    if not is_valid_hex_color(config["font_color"]):
        raise ValueError(
            f"Invalid FONT_COLOR '{config['font_color']}'. Must be a 6-digit hex code."
        )

    if not is_valid_hex_color(config["border_color"]):
        raise ValueError(
            f"Invalid BORDER_COLOR '{config['border_color']}'. Must be a 6-digit hex code."
        )
...
def apply_watermark(
    ...
    try:
        ...
        if output_path is None:
            ...
            output_filename = (
                f"{input_path_obj.stem}{SUFFIX}_{timestamp}{input_path_obj.suffix}"
            )
            output_path = str(
                Path(cfg["output_folder"] or input_path_obj.parent) / output_filename
            )
        ...
        scaled_font_size = (
            font_size if font_size is not None else int(cfg["font_size"] * scale_factor)
        )
    ...
    for file_path in files:
        try:
            ...
            output_path = apply_watermark(
                file_path, watermark_text, position=position, config=config
            )
